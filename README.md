...
用本地全量日线对六个行情策略做严格逐日事件验证：

```powershell
.\.venv\Scripts\python.exe main.py --validate-strategies --no-browser
```

默认统计信号后 5 个和 20 个交易日的净收益、中位收益、胜率、相对同日全市场等权基准的超额收益与跑赢率；可用 `--validation-horizons 5 10 20 60` 自定义周期。信号在历史上逐日计算，只使用当时已经存在的数据，下一根有效日线开盘买入，并计入佣金、卖出印花税和双边滑点。同一股票在持有周期内的连续信号只计算一次。

报告把最新一个至少覆盖120个完整交易日的年份作为留出期，证据标签要求此前开发期和留出期同时取得正中位超额收益且跑赢率超过50%。报告也按年份、板块和市场状态拆解结果。市场有利状态定义为：至少55%的股票收盘高于MA20，同时全市场20日收益中位数为正。

“均线放量 V2”“高位整理突破 V2”“多因子趋势排名”和“价格质量多因子 V1”属于研究候选，只在验证报告中出现；没有同时通过开发期与留出期前，不会加入正式选股、观察台或飞书推送。旧多因子候选每天按120/60/20日动量、趋势强度、低波动和流动性综合评分，只验证排名前20只，作为简单基准。

价格质量多因子 V1 使用120日动量（跳过最近20日）、60日动量（跳过最近5日）、趋势连续性、下行波动、距120日高点、流动性和成交稳定性评分，并扣除短期过热。它同时提供独立的每周等权组合回测：收盘计算排名、下一交易日开盘成交，新买入前20名，原持仓跌出前40名才卖出，市场过滤不通过时持有现金。组合结果展示累计收益、等权基准、相对超额、夏普、最大回撤、换手和最近一期模拟持仓。

这里的“价格质量”只描述价格路径和成交状态，不代表公司财务质量。当前数据库没有逐日财务、估值和行业历史分类，所以 V1 只能做板块内排名；后续补齐时间点正确的财务数据后，才能加入 ROE、现金流、负债、成长和估值因子。

命令只读取本地 SQLite，不访问 baostock/easy-tdx，也不发送飞书。报告输出到 `data/reports/strategy-validation-latest.html` 和同名 JSON；启动 `main.py --serve` 后可访问 `http://127.0.0.1:8765/validation`。定增策略因本地没有逐日事件快照，不进行伪历史验证。
